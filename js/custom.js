...
var initialPos =  $('#navigation').offset().top;
...
//Setup the waypoints plugin
    slide.waypoint(function (event, direction) {
        ...
        //remove the current class from the previous navigation link
        if (direction === 'down') {
            $('#navigation li[data-slide="' + dataslide + '"]').addClass('current').prev().removeClass('current');
        }
        // else If the user scrolls down change the navigation link that has the same data-slide attribute as the slide to active and
        //remove the current class from the next navigation link
        else {
    ...
    //from navigation link 2 and adds it to navigation link 1.
